...
void prv_compress_block(float **Matrix, enum Type_t type);
...
void compress_image(Image *img){
    ...
    */
    ...
    buf = 0;
    nbit = 0;
    fptr = fopen("C:\\Users\\adelc\\Desktop\\compression_v2\\test.bin","wb");
    ...
    float **Matrix = calloc_mat(BLOCK_SIZE, BLOCK_SIZE);
    enum Type_t types[3] = {LUM, COL, COL};
    for(uint8_t channel = 0; channel < 3; channel++) {
        printf("Encoding Channel %d...\n", channel);
        // compress_block
        for (int m = 0; m < num_matricies; m++) {
            prv_array_to_matrix(Matrix, &channels[channel][m * 64], m);
            prv_compress_block(Matrix, types[channel]);
        }
    }
    free_mat(Matrix);
    free_mat(channels);
...
void huffman_encode(float **Matrix, enum Type_t type){
    ...
    uint8_t fetch_bit = (uint8_t)(floor(log2(output[0])) + 1);
    uint8_t sum_index = 0;
